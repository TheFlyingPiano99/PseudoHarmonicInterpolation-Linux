...
int main(int argc, char **argv) {
	...
	Geometry::ModifiedGordonWixomSurface surface(
		std::function<Geometry::Point2D(double)>([](double t){ double r = 2; return Geometry::Point2D(r * std::cos(t), r * std::sin(t)); }),
		std::function<double(Geometry::Point2D)>([](Geometry::Point2D p){ return std::sin(p[0]); })
	);
